...
class EEGDataExtractorV2:
    ...
    def extract_raw_data(self) -> Tuple[List[Dict[str, Any]], List[Dict[str, Any]]]:
        """
        Loads the continuous preprocessed EEG data for each file and returns a list of dictionaries
        containing the participant_id and raw_data as a numpy array of shape (n_channels, n_samples).
        Also returns a list of participants with metadata.
        """
        files = self._read_from_dir()
        raw_results = []
        ...
        for file in files:
            ...
            # mne.io.Raw.get_data() returns an array natively shaped (n_channels, n_samples)
            raw_data = eeg.get_data()

            raw_results.append(
                {"participant_id": participant_id, "raw_data": raw_data}
            )
            participants.append({"participant_id": participant_id, "file": file})
...
if __name__ == "__main__":
    ...
    # Copy paste the channels from the participant to achieve a sample shape of (64, n_samples) for testing purposes
    target_channel_count = 64
    composite_raw = np.tile(sample_raw, (target_channel_count // sample_raw.shape[0] + 1, 1))[:target_channel_count, :250]
